...
class InteractiveBrowserController:
    ...
    async def start_interactive_session(self):
        ...
        print("=" * 60)
        print("Browser is now open and ready for commands!")
    ...
    async def execute_command(self, command: str):
        ...
        try:
            if cmd == 'go' and len(parts) >= 2:
                ...
                for elem in self.current_elements[:10]:
                    print(f"  {elem}")
                ...
                try:
                    ...
                    if not self.current_elements:
                        self.current_elements = await self.agent.find_clickable_elements()
                    result = await self.agent.click_element(index, self.current_elements)
                    print("Clicked element" if result else "Failed to click element")
                except ValueError:
    ...
    def show_help(self):
        ...
        print("\nAVAILABLE COMMANDS:")
        print("  go <url>              - Navigate to a website")
        print("  elements              - List all clickable elements")
        print("  click <index>         - Click element by index")
