...
class AgnoAgent:
    ...
    async def analyze_page_context(self, url: str, title: str, elements: List[ElementInfo]) -> Dict[str, Any]:
        ...
        if "login" in url_lower or "signin" in url_lower or any("password" in elem.attributes.get("type", "") for elem in elements):
            context_analysis["page_type"] = "login"
            context_analysis["primary_actions"] = ["authenticate", "enter_credentials"]
            context_analysis["security_level"] = "high"
            context_analysis["recommendations"] = ["Verify site authenticity", "Use secure connection"]
        
        elif "search" in url_lower or any("search" in elem.attributes.get("name", "").lower() for elem in elements):
            context_analysis["page_type"] = "search"
            context_analysis["primary_actions"] = ["search", "filter", "browse"]
            context_analysis["recommendations"] = ["Use specific search terms", "Apply filters if needed"]
        
        elif "checkout" in url_lower or "cart" in url_lower:
            context_analysis["page_type"] = "ecommerce"
            context_analysis["primary_actions"] = ["purchase", "review", "payment"]
            context_analysis["security_level"] = "high"
            context_analysis["recommendations"] = ["Review order details", "Verify payment security"]
        
        elif any(elem.tag_name == "form" for elem in elements):
            context_analysis["page_type"] = "form"
            context_analysis["primary_actions"] = ["fill_form", "submit"]
            context_analysis["recommendations"] = ["Fill required fields", "Review before submission"]
        
        else:
            context_analysis["page_type"] = "content"
            context_analysis["primary_actions"] = ["navigate", "read", "interact"]
            context_analysis["recommendations"] = ["Explore navigation options", "Look for relevant links"]
